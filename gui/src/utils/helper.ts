...
const stringToHTML = (arg: string) => {
  ...
  const DOM = parser.parseFromString(arg, "text/html")
  return DOM.body.childNodes[0] as HTMLElement
}

const slideUp = (el: HTMLElement, duration = 300, callback: (element: HTMLElement) => void = () => {}) => {
...
const slideDown = (el: HTMLElement, duration = 300, callback: (el: HTMLElement) => void = () => {}) => {
...
const debounce = <T>(callback: (args: T) => void, delay = 200) => {
...
const extractExternalResults = (
  error: AxiosError<{ data: { error: Array<{ path: string; message: string }> } }>
): Record<string, string> | null => {
  if (!error.response) return null
  if (error.response.status === 422) {
    if (Array.isArray(error.response.data.data.error)) {
      return error.response.data.data.error.reduce((acc, error) => {
...
const extractValidationError = (
  error: AxiosError<{ data: { error: Array<{ path: string; message: string }> } }>
): { paths: Array<string>; errorMessages: Array<string> } | null => {
  if (!error.response) return null
  if (error.response.status === 422) {
    const paths: Array<string> = []
    const errorMessages: Array<string> = []
    if (Array.isArray(error.response.data.data.error)) {
      error.response.data.data.error.forEach((error) => {
        if (error.path) paths.push(error.path)
        errorMessages.push(`${error.path}: ${error.message}`)
      })
    }
    return { paths, errorMessages }
  } else return null
}

// Checks if object/array has any keys
const isObjectEmpty = (target: object): boolean => {
