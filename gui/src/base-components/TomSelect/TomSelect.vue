...
const tomSelect = ref<TomSelectPlugin | undefined>()
...
const vSelectDirective = {
  mounted(el: TomSelectElement) {
    ...
    tomSelect.value = init(el, clonedEl, props, computedOptions.value, emit)
  },
...
<template>
  <select ref="tomSelectRef" class="tom-select" v-select-directive>
    <slot></slot>
  </select>
</template>
