...
const mapFieldsForDisplay = () => {
  if (resourceData.platform && resourceData.platform.title === 'AWS') {
    ...
    if (resourceData.excludedInstanceTypes && typeof resourceData.excludedInstanceTypes === 'string') {
      ...
      resourceData.excludedInstanceTypes = _.filter(availableInstanceTypes.value, (instanceType: IType) => excludedInstanceTypesIds.includes(instanceType.instanceType))
    }
  }
}
